...
class Constraints1 extends React.Component<Props, State> {
  ...
  public validate(newConstraint1StartDateName: string, newConstraint1StopDateName: string, newConstraint1MinNumberOfAssignments: number): ErrorMessages {
    const errorMessages: ErrorMessages = {
      newConstraint1MinNumberOfAssignments: [],
      newConstraint1StartDateName: [],
      newConstraint1StopDateName: [],
    }
    if (!utils.stringToDate(newConstraint1StartDateName)) { errorMessages.newConstraint1StartDateName.push('開始日の形式が正しくありません') }
    if (!utils.stringToDate(newConstraint1StopDateName)) { errorMessages.newConstraint1StopDateName.push('終了日の形式が正しくありません') }
    if (isNaN(newConstraint1MinNumberOfAssignments)) { errorMessages.newConstraint1MinNumberOfAssignments.push('割り当て職員数下限の形式が正しくありません') }
      ...
      <>
        <div className={this.props.classes.gridFrame}>
          <Grid container={true} spacing={8}>
            <Grid item={true} xs={12}>
              <Toolbar>
                <Typography variant="subheading" className={this.props.classes.toolbarTitle}> 期間の勤務にグループから割り当てる職員数の下限</Typography>
        ...
        {this.props.kinmus.length === 0 || this.props.groups.length === 0 ?
          ...
          (() => {
            ...
            const errorMessages = this.validate(this.state.newConstraint1StartDateName, this.state.newConstraint1StopDateName, this.state.newConstraint1MinNumberOfAssignments)
            return (
              <Dialog onClose={this.handleCloseCreationDialog} open={this.state.creationDialogIsOpen} fullWidth={true} maxWidth="md">
                  ...
                  <Grid container={true} spacing={8}>
                    ...
                    <Grid item={true} xs={12}>
                      <TextField
                        ...
                        error={errorMessages.newConstraint1StartDateName.length > 0}
                        FormHelperTextProps={{
                          component: 'div',
                        }}
                        helperText={errorMessages.newConstraint1StartDateName.map(message =>
                    ...
                    <Grid item={true} xs={12}>
                      <TextField
                        ...
                        error={errorMessages.newConstraint1StopDateName.length > 0}
                        FormHelperTextProps={{
                          component: 'div',
                        }}
                        helperText={errorMessages.newConstraint1StopDateName.map(message =>
                          <div key={message}>{message}</div>
                        )}
                      />
                    </Grid>
                    ...
                    <Grid item={true} xs={12}>
                      <TextField
                        ...
                        error={errorMessages.newConstraint1MinNumberOfAssignments.length > 0}
                        FormHelperTextProps={{
                          component: 'div',
                        }}
                        helperText={errorMessages.newConstraint1MinNumberOfAssignments.map(message =>
                          <div key={message}>{message}</div>
                        )}
                      />
                    </Grid>
