...
class Constraint5 extends React.Component<Props, State> {
  ...
  public validate(constraint5MinNumberOfDays: number): ErrorMessages {
    ...
    if (isNaN(constraint5MinNumberOfDays)) { errorMessages.constraint5MinNumberOfDays.push('連続日数下限の形式が正しくありません') }
  ...
  public render() {
    const constraint5Kinmu = this.props.kinmus.find(({ id }) => id === this.props.constraint5.kinmu_id)!
      ...
      <>
        <Card>
          <CardHeader
            avatar={
              <Switch
                checked={this.props.constraint5.is_enabled && relativesAreEnabled}
