...
class Constraints2 extends React.Component<Props, State> {
  ...
  public validate(newConstraint2StartDateName: string, newConstraint2StopDateName: string, newConstraint2MaxNumberOfAssignments: number): ErrorMessages {
    const errorMessages: ErrorMessages = {
      newConstraint2MaxNumberOfAssignments: [],
      newConstraint2StartDateName: [],
      newConstraint2StopDateName: [],
    }
    if (!utils.stringToDate(newConstraint2StartDateName)) { errorMessages.newConstraint2StartDateName.push('開始日の形式が正しくありません') }
    if (!utils.stringToDate(newConstraint2StopDateName)) { errorMessages.newConstraint2StopDateName.push('終了日の形式が正しくありません') }
    if (isNaN(newConstraint2MaxNumberOfAssignments)) { errorMessages.newConstraint2MaxNumberOfAssignments.push('割り当て職員数上限の形式が正しくありません') }
      ...
      <>
        ...
        {this.props.kinmus.length === 0 || this.props.groups.length === 0 ?
          ...
          (() => {
            ...
            const newConstraint2StartDateIsEnabled = newConstraint2StartDate ? this.props.terms.every(({ start_date_name }) => {
            ...
            const newConstraint2StopDateIsEnabled = newConstraint2StopDate ? this.props.terms.every(({ stop_date_name }) => {
            ...
            const relativesAreEnabled = newConstraint2StartDateIsEnabled && newConstraint2StopDateIsEnabled && newConstraint2Kinmu.is_enabled && newConstraint2Group.is_enabled
            const errorMessages = this.validate(this.state.newConstraint2StartDateName, this.state.newConstraint2StopDateName, this.state.newConstraint2MaxNumberOfAssignments)
            return (
              <Dialog onClose={this.handleCloseCreationDialog} open={this.state.creationDialogIsOpen} fullWidth={true} maxWidth="md">
                  ...
                  <Grid container={true} spacing={8}>
                    ...
                    <Grid item={true} xs={12}>
                      <TextField
                        ...
                        error={errorMessages.newConstraint2StartDateName.length > 0}
                        FormHelperTextProps={{
                          component: 'div',
                        }}
                        helperText={errorMessages.newConstraint2StartDateName.map(message =>
                    ...
                    <Grid item={true} xs={12}>
                      <TextField
                        ...
                        error={errorMessages.newConstraint2StopDateName.length > 0}
                        FormHelperTextProps={{
                          component: 'div',
                        }}
                        helperText={errorMessages.newConstraint2StopDateName.map(message =>
                          <div key={message}>{message}</div>
                        )}
                      />
                    </Grid>
                    ...
                    <Grid item={true} xs={12}>
                      <TextField
                        ...
                        error={errorMessages.newConstraint2MaxNumberOfAssignments.length > 0}
                        FormHelperTextProps={{
                          component: 'div',
                        }}
                        helperText={errorMessages.newConstraint2MaxNumberOfAssignments.map(message =>
                          <div key={message}>{message}</div>
                        )}
                      />
                    </Grid>
