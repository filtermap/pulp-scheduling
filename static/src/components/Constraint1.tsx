...
class Constraint1 extends React.Component<Props, State> {
  ...
  public handleChangeConstraint1IsEnabled = (
    _: React.ChangeEvent<HTMLInputElement>,
    checked: boolean
  ) => {
    this.props.dispatch(
      constraints1.updateConstraint1IsEnabled(
        this.props.constraint1.id,
        checked
      )
    );
  };
  ...
  public handleChangeConstraint1StartDateName = (
  ...
  public handleChangeConstraint1StopDateName = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
  ...
  public handleChangeConstraint1KinmuId = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
  ...
  public handleChangeConstraint1GroupId = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
  ...
  public handleChangeConstraint1MinNumberOfAssignments = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
  ...
  public handleClickDeleteConstraint1 = () => {
    this.setState({ deletionDialogIsOpen: false });
    this.props.dispatch(
      constraints1.deleteConstraint1(this.props.constraint1.id)
    );
  };
  public render() {
    const constraint1StartDate = utils.stringToDate(
      this.props.constraint1.start_date_name
    );
    ...
    const constraint1StopDate = utils.stringToDate(
      this.props.constraint1.stop_date_name
    );
    const constraint1StopDateIsEnabled = constraint1StopDate
      ...
      : false;
    const constraint1Kinmu = this.props.kinmus.find(
      ({ id }) => id === this.props.constraint1.kinmu_id
    )!;
    const constraint1Group = this.props.groups.find(
      ({ id }) => id === this.props.constraint1.group_id
    )!;
      ...
      <>
        <span
          className={classnames({
            [this.props.classes.lineThrough]: !constraint1StartDateIsEnabled,
          })}
        >
          {this.props.constraint1.start_date_name}
        </span>
        ...
        <span
          className={classnames({
            [this.props.classes.lineThrough]: !constraint1StopDateIsEnabled,
          })}
        >
          {this.props.constraint1.stop_date_name}
        </span>
        ...
        <span
          className={classnames({
            [this.props.classes.lineThrough]: !constraint1Group.is_enabled,
          })}
        >
          {constraint1Group.name}
        </span>
        から{this.props.constraint1.min_number_of_assignments}
        人以上の職員を割り当てる
      </>
    );
    const errorMessages = this.validate(
      this.props.constraint1.start_date_name,
      this.props.constraint1.stop_date_name,
      this.props.constraint1.min_number_of_assignments
    );
    return (
      <>
        <Card>
          <CardHeader
            avatar={
              <Switch
                checked={
                  this.props.constraint1.is_enabled && relativesAreEnabled
                }
              ...
              <Grid container={true} spacing={8}>
                <Grid item={true} xs={12}>
                  <TextField
                    ...
                    inputProps={{
                      className: classnames({
                        [this.props.classes
                          .lineThrough]: !constraint1StartDateIsEnabled,
                      }),
                    }}
                ...
                <Grid item={true} xs={12}>
                  <TextField
                    ...
                    inputProps={{
                      className: classnames({
                        [this.props.classes
                          .lineThrough]: !constraint1StopDateIsEnabled,
                      }),
                    }}
                ...
                <Grid item={true} xs={12}>
                  <TextField
                    ...
                    onChange={
                      this.handleChangeConstraint1MinNumberOfAssignments
                    }
                    fullWidth={true}
                    inputProps={{
        ...
        >
          ...
          <DialogActions>
            <Button color="primary" onClick={this.handleClickDeleteConstraint1}>
              削除
            </Button>
