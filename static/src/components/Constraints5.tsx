...
class Constraints5 extends React.Component<Props, State> {
  ...
  public validate(newConstraint5MinNumberOfDays: number): ErrorMessages {
    ...
    if (isNaN(newConstraint5MinNumberOfDays)) { errorMessages.newConstraint5MinNumberOfDays.push('連続日数下限の形式が正しくありません') }
    ...
    return (
      <>
        <div className={this.props.classes.gridFrame}>
          <Grid container={true} spacing={8}>
            <Grid item={true} xs={12}>
              <Toolbar>
                <Typography variant="subheading" className={this.props.classes.toolbarTitle}>勤務の連続日数の下限</Typography>
        ...
        {this.props.kinmus.length === 0 ?
          ...
          (() => {
            const newConstraint5Kinmu = this.props.kinmus.find(({ id }) => id === this.state.newConstraint5KinmuId)!
            const relativesAreEnabled = newConstraint5Kinmu.is_enabled
            const errorMessages = this.validate(this.state.newConstraint5MinNumberOfDays)
            return (
              <Dialog onClose={this.handleCloseCreationDialog} open={this.state.creationDialogIsOpen} fullWidth={true} maxWidth="md">
                  ...
                  <Grid container={true} spacing={8}>
                    <Grid item={true} xs={12}>
                      <FormControlLabel
                        control={
                          <Switch
                            checked={this.state.newConstraint5IsEnabled && relativesAreEnabled}
                    ...
                    <Grid item={true} xs={12}>
                      <TextField
                        ...
                        error={errorMessages.newConstraint5MinNumberOfDays.length > 0}
                        FormHelperTextProps={{
                          component: 'div',
                        }}
                        helperText={errorMessages.newConstraint5MinNumberOfDays.map(message =>
