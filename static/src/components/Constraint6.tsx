...
class Constraint6 extends React.Component<Props, State> {
  ...
  public handleChangeConstraint6IsEnabled = (
    _: React.ChangeEvent<HTMLInputElement>,
    checked: boolean
  ) => {
    this.props.dispatch(
      constraints6.updateConstraint6IsEnabled(
        this.props.constraint6.id,
        checked
      )
    );
  };
  public handleChangeConstraint6KinmuId = (
  ...
  public handleChangeConstraint6MaxNumberOfDays = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
  ...
  public handleClickDeleteConstraint6 = () => {
    this.setState({ deletionDialogIsOpen: false });
    this.props.dispatch(
      constraints6.deleteConstraint6(this.props.constraint6.id)
    );
  };
  public render() {
    const constraint6Kinmu = this.props.kinmus.find(
      ({ id }) => id === this.props.constraint6.kinmu_id
    )!;
      ...
      <>
        <span
          className={classnames({
            [this.props.classes.lineThrough]: !constraint6Kinmu.is_enabled,
          })}
        >
          {constraint6Kinmu.name}
        </span>
        の連続日数を{this.props.constraint6.max_number_of_days}日以下にする
      </>
    );
    const errorMessages = this.validate(
      this.props.constraint6.max_number_of_days
    );
    return (
      <>
        <Card>
          <CardHeader
            avatar={
              <Switch
                checked={
                  this.props.constraint6.is_enabled && relativesAreEnabled
                }
        ...
        >
          ...
          <DialogActions>
            <Button color="primary" onClick={this.handleClickDeleteConstraint6}>
              削除
            </Button>
