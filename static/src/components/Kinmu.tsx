...
class Kinmu extends React.Component<Props, State> {
  ...
  public handleChangeKinmuIsEnabled = (
    _: React.ChangeEvent<HTMLInputElement>,
    checked: boolean
  ) => {
    this.props.dispatch(
      kinmus.updateKinmuIsEnabled(this.props.kinmu.id, checked)
    );
  };
  ...
  public handleChangeKinmuName = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
      kinmus.updateKinmuName(this.props.kinmu.id, event.target.value)
    );
  };
  ...
  public render() {
    ...
    const kinmuConstraint0Ids = Array.from(
      new Set(
        this.props.constraint0_kinmus
          .filter(({ kinmu_id }) => kinmu_id === this.props.kinmu.id)
          .map(({ constraint0_id }) => constraint0_id)
      )
    );
    const kinmuConstraints1 = this.props.constraints1.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints2 = this.props.constraints2.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints3 = this.props.constraints3.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints4 = this.props.constraints4.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints5 = this.props.constraints5.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints6 = this.props.constraints6.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints7 = this.props.constraints7.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints8 = this.props.constraints8.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints9 = this.props.constraints9.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    const kinmuConstraints10 = this.props.constraints10.filter(
      (c) => c.kinmu_id === this.props.kinmu.id
    );
    ...
    return (
      <>
        ...
        >
            ...
            <Grid container={true} spacing={1}>
              ...
              <Grid item={true} xs={12}>
                ...
                {kinmuConstraints5.map((c) => (
                  <Typography
                    key={c.id}
                  >{`${this.props.kinmu.name}の連続日数を${c.min_number_of_days}日以上にする`}</Typography>
                ))}
                {kinmuConstraints6.map((c) => (
                  <Typography
                    key={c.id}
                  >{`${this.props.kinmu.name}の連続日数を${c.max_number_of_days}日以下にする`}</Typography>
                ))}
                {kinmuConstraints7.map((c) => (
                  <Typography
                    key={c.id}
                  >{`${this.props.kinmu.name}の間隔日数を${c.min_number_of_days}日以上にする`}</Typography>
                ))}
                {kinmuConstraints8.map((c) => (
                  <Typography
                    key={c.id}
                  >{`${this.props.kinmu.name}の間隔日数を${c.max_number_of_days}日以下にする`}</Typography>
                ))}
          ...
          <DialogActions>
            <Button color="primary" onClick={this.handleClickDeleteKinmu}>
              削除
            </Button>
