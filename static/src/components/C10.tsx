...
function C10(props: Props) {
  ...
  function handleClickDeleteC10(index: number) {
    return (_: React.MouseEvent<HTMLButtonElement>) => {
      props.dispatch(c10.deleteC10(index))
    }
  }
  return (
    <>
      <Toolbar>
        <Typography variant="subheading">職員の期間に割り当てない勤務</Typography>
      </Toolbar>
      {props.c10.map(c => (
        <ExpansionPanel key={c.index}>
          ...
          <ExpansionPanelActions>
            <Button size="small" onClick={handleClickDeleteC10(c.index)}>削除</Button>
          </ExpansionPanelActions>
        </ExpansionPanel>
      ))}
    </>
