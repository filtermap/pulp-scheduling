...
class Member extends React.Component<Props, State> {
  ...
  public handleChangeGroupMember(groupId: number) {
    return (_: React.ChangeEvent<HTMLInputElement>, checked: boolean) => {
      if (checked) {
        this.props.dispatch(group_members.createGroupMember(groupId, this.props.member.id))
        return
      }
      this.props.dispatch(group_members.deleteGroupMember(groupId, this.props.member.id))
    }
  }
  ...
  public render() {
    const memberRosterIds = Array.from(new Set(this.props.assignments.filter(({ member_id }) => member_id === this.props.member.id).map(({ roster_id }) => roster_id)))
    const memberConstraints3 = this.props.constraints3.filter(c => c.member_id === this.props.member.id)
                    ...
                    <FormGroup>
                      {this.props.groups.map(group => (
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              checked={this.props.group_members.some(group_member => group_member.group_id === group.id && group_member.member_id === this.props.member.id)}
          ...
          <DialogContent>
            <Grid container={true} spacing={8}>
              <Grid item={true} xs={12}>
                ...
                <Typography variant="caption">{this.props.group_members.filter(group_member => group_member.member_id === this.props.member.id).map(group_member => this.props.groups.find(group => group.id === group_member.group_id)!.name).join(', ')}</Typography>
              </Grid>
              <Grid item={true} xs={12}>
                {memberRosterIds.length > 0 && <DialogContentText>以下の勤務表の割り当ても削除されます</DialogContentText>}
                {memberRosterIds.map(roster_id => <Typography key={roster_id}>{`勤務表${roster_id}`}</Typography>)}
              </Grid>
              <Grid item={true} xs={12}>
                {(memberConstraints3.length > 0 || memberConstraints4.length > 0 || memberConstraints9.length > 0 || memberConstraints10.length > 0) &&
                  <DialogContentText>以下の条件も削除されます</DialogContentText>}
                {memberConstraints3.map(c => <Typography key={c.id}>{`${this.props.member.name}に${this.props.kinmus.find(kinmu => kinmu.id === c.kinmu_id)!.name}を${c.min_number_of_assignments}回以上割り当てる`}</Typography>)}
                {memberConstraints4.map(c => <Typography key={c.id}>{`${this.props.member.name}に${this.props.kinmus.find(kinmu => kinmu.id === c.kinmu_id)!.name}を${c.max_number_of_assignments}回以下割り当てる`}</Typography>)}
                {memberConstraints9.map(c => <Typography key={c.id}>{`${this.props.member.name}の${c.start_date_name}から${c.stop_date_name}までに${this.props.kinmus.find(kinmu => kinmu.id === c.kinmu_id)!.name}を割り当てる`}</Typography>)}
                {memberConstraints10.map(c => <Typography key={c.id}>{`${this.props.member.name}の${c.start_date_name}から${c.stop_date_name}までに${this.props.kinmus.find(kinmu => kinmu.id === c.kinmu_id)!.name}を割り当てない`}</Typography>)}
              </Grid>
            </Grid>
          </DialogContent>
          <DialogActions>
