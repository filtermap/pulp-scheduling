...
class Rosters extends React.Component<Props, State> {
  ...
  public handleClickCreateRoster = () => {
    ...
    if (this.state.dialogState.type === SOLVED) {
      this.props.dispatch(all.createRoster(this.state.dialogState.newRosterAssignments))
    }
  }
  public dialog = () => {
    switch (this.state.dialogState.type) {
      ...
      case SOLVED: {
        ...
        const newRosterDateNames = sortDateNames(Array.from(new Set(newRosterAssignments.map(({ date_name }) => date_name))))
        const newRosterMemberIds = Array.from(new Set(newRosterAssignments.map(({ member_id }) => member_id)))
                ...
                <Table>
                  ...
                  <TableBody>
                    {newRosterMembers.map(member => {
                      const newRosterMemberAssignments = newRosterAssignments.filter(assignment => assignment.member_id === member.id)
                      return (
            ...
            <DialogContent>
              ...
              {this.state.dialogState.errorMessage === 'Infeasible' ?
                <>
                  ...
                  <Button size="small" onClick={this.handleClickPursue}>勤務表を作成できない原因となる条件を特定</Button>
                </> :
                <Typography>pulp-schedulingの不具合や条件の誤りなどにより作成できない可能性があります（{this.state.dialogState.errorMessage}）</Typography>}
            </DialogContent>
            <DialogActions>
              <Button color="primary" onClick={this.handleCloseCreationDialog}>閉じる</Button>
            </DialogActions>
      ...
      case PURSUE_IN_PROGRESS:
        return (
          <Dialog open={this.state.creationDialogIsOpen} fullWidth={true} maxWidth="md">
            <DialogTitle>勤務表の追加</DialogTitle>
            <DialogContent>
              <DialogContentText>勤務表を作成できない原因となる条件を特定中...</DialogContentText>
              <LinearProgress variant="query" />
            </DialogContent>
          </Dialog>
        )
      case PURSUED: {
        ...
        const constraintText = (() => {
          switch (constraint.type) {
            case 'Constraint0': {
              const constraint0 = this.props.all.constraints0.find(({ id }) => id === constraint.id)!
              const constraint0Constraint0Kinmus = this.props.all.constraint0_kinmus.filter(({ constraint0_id }) => constraint0_id === constraint0.id).sort((a, b) => a.sequence_number - b.sequence_number)
              const constraint0Constraint0KinmuKinmus = constraint0Constraint0Kinmus.map(({ kinmu_id }) => this.props.all.kinmus.find(kinmu => kinmu.id === kinmu_id)!)
              return constraint0Constraint0KinmuKinmus.map(kinmu => kinmu.name).join(', ')
            }
            case 'Constraint1': {
              const constraint1 = this.props.all.constraints1.find(({ id }) => id === constraint.id)!
              const constraint1Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint1.kinmu_id)!
              const constraint1Group = this.props.all.groups.find(({ id }) => id === constraint1.group_id)!
            ...
            case 'Constraint2': {
              const constraint2 = this.props.all.constraints2.find(({ id }) => id === constraint.id)!
              const constraint2Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint2.kinmu_id)!
              const constraint2Group = this.props.all.groups.find(({ id }) => id === constraint2.group_id)!
            ...
            case 'Constraint3': {
              const constraint3 = this.props.all.constraints3.find(({ id }) => id === constraint.id)!
              const constraint3Member = this.props.all.members.find(({ id }) => id === constraint3.member_id)!
              const constraint3Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint3.kinmu_id)!
              return `${constraint3Member.name}に${constraint3Kinmu.name}を${constraint3.min_number_of_assignments}回以上割り当てる`
            }
            case 'Constraint4': {
              const constraint4 = this.props.all.constraints4.find(({ id }) => id === constraint.id)!
              const constraint4Member = this.props.all.members.find(({ id }) => id === constraint4.member_id)!
              const constraint4Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint4.kinmu_id)!
              return `${constraint4Member.name}に${constraint4Kinmu.name}を${constraint4.max_number_of_assignments}回以下割り当てる`
            }
            case 'Constraint5': {
              const constraint5 = this.props.all.constraints5.find(({ id }) => id === constraint.id)!
              const constraint5Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint5.kinmu_id)!
              return `${constraint5Kinmu.name}の連続日数を${constraint5.min_number_of_days}日以上にする`
            }
            case 'Constraint6': {
              const constraint6 = this.props.all.constraints6.find(({ id }) => id === constraint.id)!
              const constraint6Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint6.kinmu_id)!
              return `${constraint6Kinmu.name}の連続日数を${constraint6.max_number_of_days}日以下にする`
            }
            case 'Constraint7': {
              const constraint7 = this.props.all.constraints7.find(({ id }) => id === constraint.id)!
              const constraint7Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint7.kinmu_id)!
              return `${constraint7Kinmu.name}の間隔日数を${constraint7.min_number_of_days}日以上にする`
            }
            case 'Constraint8': {
              const constraint8 = this.props.all.constraints8.find(({ id }) => id === constraint.id)!
              const constraint8Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint8.kinmu_id)!
              return `${constraint8Kinmu.name}の間隔日数を${constraint8.max_number_of_days}日以下にする`
            }
            case 'Constraint9': {
              const constraint9 = this.props.all.constraints9.find(({ id }) => id === constraint.id)!
              const constraint9Member = this.props.all.members.find(({ id }) => id === constraint9.member_id)!
              const constraint9Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint9.kinmu_id)!
            ...
            case 'Constraint10': {
              const constraint10 = this.props.all.constraints10.find(({ id }) => id === constraint.id)!
              const constraint10Member = this.props.all.members.find(({ id }) => id === constraint10.member_id)!
              const constraint10Kinmu = this.props.all.kinmus.find(({ id }) => id === constraint10.kinmu_id)!
        ...
        return (
          <Dialog onClose={this.handleCloseCreationDialog} open={this.state.creationDialogIsOpen} fullWidth={true} maxWidth="md">
            <DialogTitle>勤務表の追加</DialogTitle>
            <DialogContent>
              <DialogContentText>以下の条件により勤務表を作成できませんでした</DialogContentText>
      ...
      case UNPURSUED:
        return (
          <Dialog onClose={this.handleCloseCreationDialog} open={this.state.creationDialogIsOpen} fullWidth={true} maxWidth="md">
            <DialogTitle>勤務表の追加</DialogTitle>
            <DialogContent>
              <DialogContentText>勤務表を作成できない原因となる条件を特定できませんでした</DialogContentText>
              <Typography>pulp-schedulingの不具合や条件の誤りなどにより特定できない可能性があります（{this.state.dialogState.errorMessage}）</Typography>}
            </DialogContent>
            <DialogActions>
              <Button color="primary" onClick={this.handleCloseCreationDialog}>閉じる</Button>
            </DialogActions>
