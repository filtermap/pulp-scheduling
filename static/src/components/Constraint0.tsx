...
class Constraint0 extends React.Component<Props, State> {
  ...
  public handleChangeConstraint0IsEnabled = (_: React.ChangeEvent<HTMLInputElement>, checked: boolean) => {
    this.props.dispatch(constraints0.updateConstraint0IsEnabled(this.props.constraint0.id, checked))
  }
  public handleClickCreateConstraint0Kinmu(sequence_number: number) {
    return () => {
      const kinmu_id = this.props.kinmus[0].id
      this.props.dispatch(constraint0_kinmus.createConstraint0Kinmu(this.props.constraint0.id, sequence_number, kinmu_id))
    }
  }
  public handleChangeConstraint0KinmuKinmuId(constraint0_kinmu_id: number) {
    return (event: React.ChangeEvent<HTMLInputElement>) => {
      this.props.dispatch(constraint0_kinmus.updateConstraint0KinmuKinmuId(constraint0_kinmu_id, parseInt(event.target.value, 10)))
    }
  }
            ...
            <CardContent>
              <Grid container={true} spacing={8}>
                <Grid item={true} xs={12}>
                  <Button size="small" onClick={this.handleClickCreateConstraint0Kinmu(0)}>追加</Button>
                </Grid>
                {constraint0Constraint0Kinmus.map((constraint0_kinmu, index) => (
                  <React.Fragment key={constraint0_kinmu.id}>
                    ...
                    <Grid item={true} xs={12}>
                      <Button size="small" onClick={this.handleClickCreateConstraint0Kinmu(constraint0_kinmu.sequence_number + 1)}>追加</Button>
                    </Grid>
                  </React.Fragment>
                ))}
              </Grid>
