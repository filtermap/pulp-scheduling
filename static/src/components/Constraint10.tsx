...
class Constraint10 extends React.Component<Props, State> {
  ...
  public handleChangeConstraint10IsEnabled = (
    _: React.ChangeEvent<HTMLInputElement>,
    checked: boolean
  ) => {
    this.props.dispatch(
      constraints10.updateConstraint10IsEnabled(
        this.props.constraint10.id,
        checked
      )
    );
  };
  public handleChangeConstraint10MemberId = (
  ...
  public handleChangeConstraint10StartDateName = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
  ...
  public handleChangeConstraint10StopDateName = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
  ...
  public handleChangeConstraint10KinmuId = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    this.props.dispatch(
  ...
  public handleClickDeleteConstraint10 = () => {
    this.setState({ deletionDialogIsOpen: false });
    this.props.dispatch(
      constraints10.deleteConstraint10(this.props.constraint10.id)
    );
  };
  public render() {
    const constraint10Member = this.props.members.find(
      ({ id }) => id === this.props.constraint10.member_id
    )!;
    const constraint10StartDate = utils.stringToDate(
      this.props.constraint10.start_date_name
    );
    ...
    const constraint10StopDate = utils.stringToDate(
      this.props.constraint10.stop_date_name
    );
    const constraint10StopDateIsEnabled = constraint10StopDate
      ...
      : false;
    const constraint10Kinmu = this.props.kinmus.find(
      ({ id }) => id === this.props.constraint10.kinmu_id
    )!;
      ...
      <>
        <span
          className={classnames({
            [this.props.classes.lineThrough]: !constraint10Member.is_enabled,
          })}
        >
        ...
        <span
          className={classnames({
            [this.props.classes.lineThrough]: !constraint10StartDateIsEnabled,
          })}
        >
          {this.props.constraint10.start_date_name}
        </span>
        ...
        <span
          className={classnames({
            [this.props.classes.lineThrough]: !constraint10StopDateIsEnabled,
          })}
        >
          {this.props.constraint10.stop_date_name}
        </span>
        ...
        <span
          className={classnames({
            [this.props.classes.lineThrough]: !constraint10Kinmu.is_enabled,
          })}
        >
      ...
      <>
        <Card>
          <CardHeader
            avatar={
              <Switch
                checked={
                  this.props.constraint10.is_enabled && relativesAreEnabled
                }
        ...
        >
          ...
          <DialogActions>
            <Button
              color="primary"
              onClick={this.handleClickDeleteConstraint10}
            >
              削除
            </Button>
