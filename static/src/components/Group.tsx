...
class Group extends React.Component<Props, State> {
  ...
  public handleChangeGroupMember(memberId: number) {
    return (_: React.ChangeEvent<HTMLInputElement>, checked: boolean) => {
      if (checked) {
        this.props.dispatch(group_members.createGroupMember(this.props.group.id, memberId))
        return
      }
      this.props.dispatch(group_members.deleteGroupMember(this.props.group.id, memberId))
    }
  }
      ...
      <>
        <Card>
          <CardHeader
            ...
            subheader={this.props.group_members.filter(group_member => group_member.group_id === this.props.group.id).map(group_member => this.props.members.find(member => member.id === group_member.member_id)!.name).join(', ')}
          />
                    ...
                    <FormGroup>
                      {this.props.members.map(member => (
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              checked={this.props.group_members.some(group_member => group_member.group_id === this.props.group.id && group_member.member_id === member.id)}
          ...
          <DialogContent>
            <Grid container={true} spacing={8}>
              <Grid item={true} xs={12}>
                ...
                <Typography variant="caption">{this.props.group_members.filter(group_member => group_member.group_id === this.props.group.id).map(group_member => this.props.members.find(member => member.id === group_member.member_id)!.name).join(', ')}</Typography>
              </Grid>
              <Grid item={true} xs={12}>
                {(groupConstraints1.length > 0 || groupConstraints2.length > 0) &&
                  <DialogContentText>以下の条件も削除されます</DialogContentText>}
                {groupConstraints1.map(c => <Typography key={c.id}>{`${c.start_date_name}から${c.stop_date_name}までの${this.props.kinmus.find(({ id }) => id === c.kinmu_id)!.name}に${this.props.group.name}から${c.min_number_of_assignments}人以上の職員を割り当てる`}</Typography>)}
                {groupConstraints2.map(c => <Typography key={c.id}>{`${c.start_date_name}から${c.stop_date_name}までの${this.props.kinmus.find(({ id }) => id === c.kinmu_id)!.name}に${this.props.group.name}から${c.max_number_of_assignments}人以下の職員を割り当てる`}</Typography>)}
              </Grid>
            </Grid>
          </DialogContent>
          <DialogActions>
